...
if (in_array($http_code, [301, 302, 303, 307, 308]) && isset($response_headers['location'])) {
    ...
    header("Location: ysws.php?url=" . urlencode($location), true, $http_code);
    exit();
}
...
if ($is_m3u8) {
    ...
    $body = preg_replace_callback(
        ...
        function ($matches) use ($base_root, $base_dir) {
            ...
            // 跳过已经被代理过的
            if (str_contains($url, 'ysws.php?url=')) return $url;
    
            // 完整 URL
            if (preg_match('/^https?:\/\//i', $url)) {
                return 'ysws.php?url=' . urlencode($url);
            }
    
            // 以 / 开头（绝对路径）
            if (str_starts_with($url, '/')) {
                return 'ysws.php?url=' . urlencode($base_root . $url);
            }
    
            // 处理 ../ 或 ./ 等相对路径
            return 'ysws.php?url=' . urlencode($base_dir . $url);
        },
        $body
    );
